<script>
  ...
  defaultOptions.projectId =
    projectData.projectId || `p4-${projectData.uniqueId}`;
  for (const variable of cloudVariables) {
    defaultOptions.cloudVariables.custom[variable] = "ws";
  }
  defaultOptions.app.packageName = Packager.getDefaultPackageNameFromFileName(
    projectData.title
  );
  defaultOptions.app.windowTitle = Packager.getWindowTitleFromFileName(
    projectData.title
  );
  defaultOptions.extensions = projectData.project.analysis.extensions;
  const options = writablePersistentStore(
    `PackagerOptions.${projectData.uniqueId}`,
    defaultOptions
  );
  ...
  const hasMagicComment = (magic) =>
    projectData.project.analysis.stageComments.find((text) =>
  ...
  const icon = fileStore.writableFileStore(
    `PackagerOptions.icon.${projectData.uniqueId}`
  );
  $: $options.app.icon = $icon;

  const customCursorIcon = fileStore.writableFileStore(
    `PackagerOptions.customCursorIcon.${projectData.uniqueId}`
  );
  $: $options.cursor.custom = $customCursorIcon;

  const loadingScreenImage = fileStore.writableFileStore(
    `PackagerOptions.loadingScreenImage.${projectData.uniqueId}`
  );
    ...
    packager.addEventListener("large-asset-fetch", ({ detail }) => {
      ...
      if (thing) {
        task.setProgressText(
          $_("progress.loadingLargeAsset").replace("{thing}", thing)
        );
      }
      task.setProgress(detail.progress);
    });
  ...
  const exportOptions = async () => {
    ...
    const formattedAppName = APP_NAME.replace(/[^a-z0-9 ]/gi, "")
      .replace(/ /g, "-")
      .toLowerCase();
  ...
  <div>
    ...
    <label class="option">
      ...
      <input
        ...
        max="4096"
        step="1"
        bind:value={$options.stageHeight}
      />
...
>
  <div>
    ...
    <label class="option">
      <input
        type="checkbox"
        bind:checked={$options.loadingScreen.progressBar}
      />
    ...
    <label class="option">
      ...
      <input
        type="text"
        bind:value={$options.loadingScreen.text}
    ...
    {#if $loadingScreenImage}
      <label class="option">
        <input
          ...
          value="normal"
          bind:group={$options.loadingScreen.imageMode}
        />
    ...
    <label class="option">
      <input
        type="checkbox"
        bind:checked={$options.controls.greenFlag.enabled}
      />
      {$_("options.showFlag")}
    </label>
    <label class="option">
    ...
    <label class="option">
      <input
        type="checkbox"
        bind:checked={$options.controls.fullscreen.enabled}
      />
    ...
    <div class="group">
      <label class="option">
        <input
          type="radio"
          name="cursor-type"
          bind:group={$options.cursor.type}
          value="auto"
        />
    ...
    {#if $options.cursor.type === "custom"}
      <div in:slide|self class="option">
        ...
        <label class="option">
          ...
          X:
          <input type="number" min="0" bind:value={$options.cursor.center.x} />
          Y:
          <input type="number" min="0" bind:value={$options.cursor.center.y} />
          <button
    ...
    {#if cloudVariables.length > 0}
          ...
          {#each cloudVariables as variable}
            <label class="option">
              <select bind:value={$options.cloudVariables.custom[variable]}>
                <option value="ws">{$_("options.cloudVariables-ws")}</option>
                <option value="local"
                  >{$_("options.cloudVariables-local")}</option
                >
                <option value="">{$_("options.cloudVariables-ignore")}</option>
              </select>
              {variable}
      ...
      {#if $options.cloudVariables.mode === "ws" || $options.cloudVariables.mode === "custom"}
        <div transition:fade|local>
          <label class="option">
            ...
            <input
              type="text"
              bind:value={$options.cloudVariables.cloudHost}
              pattern="wss?:.*"
            />
          </label>
        </div>
      ...
      <div class="option">
        <label>
          <input
            type="checkbox"
            bind:checked={$options.cloudVariables.specialCloudBehaviors}
          />
      ...
      <div class="option">
        <label>
          <input
            type="checkbox"
            bind:checked={$options.cloudVariables.unsafeCloudBehaviors}
          />
...
>
  <div>
    ...
    <details open={advancedOptionsInitiallyOpen}>
      ...
      <label class="option">
        {$_("options.customCSS")}
        <textarea bind:value={$options.custom.css} />
      </label>
      <label class="option">
        {$_("options.customJS")}
        <textarea bind:value={$options.custom.js} />
      </label>
      ...
      <label class="option">
        <input
          type="checkbox"
          checked={$options.maxTextureDimension !==
            defaultOptions.maxTextureDimension}
          on:change={(e) => {
            $options.maxTextureDimension =
              defaultOptions.maxTextureDimension * (e.target.checked ? 2 : 1);
          }}
        />
...
{#if $options.target !== "html"}
  <div in:fade|local>
    <Section
      accent="#FF661A"
      reset={$options.target.startsWith("zip")
        ? null
        : () => {
    ...
    >
      <div>
        {#if $options.target.startsWith("zip")}
          <h2>Zip</h2>
          <p>
            The zip environment is intended to be used for publishing to a
            website. Other uses such as sending your project to a friend over a
            chat app or email should use "Plain HTML" instead as zip will not
            work.
          </p>
        {:else}
          ...
          <label class="option">
            ...
            <input
              type="text"
              bind:value={$options.app.version}
              pattern="\d+\.\d+\.\d+"
              minlength="1"
            />
          </label>
            ...
            <div class="group">
              <label class="option">
                <input
                  ...
                  bind:group={$options.app.windowMode}
                  value="window"
                />
          ...
          <div class="warning">
            <div>
              Creating native applications for specific platforms is
              discouraged. In most cases, Plain HTML or Zip will have numerous
              advantages:
            </div>
            <ul>
              <li>
            ...
            <div>
              If you don't truly need to make a self-contained application for
              each platform (we understand there are some cases where this is
              necessary), we recommend you don't.
            </div>
          </div>
          ...
          {#if $options.target.includes("win")}
            <div>
              <h2>Windows</h2>
              <p>
                All Windows applications generated by this site are unsigned, so
                users will see SmartScreen warnings when they try to run it for
                the first time. They can bypass these warnings by pressing "More
                info" then "Run anyways".
              </p>
              <!-- !!! CHANGE !!! -->
              <!-- <p>
                To change the icon of the executable file or create an installer
                program, download and run <a
                  href="https://github.com/TurboWarp/packager-extras/releases"
                  >TurboWarp Packager Extras</a
                > and select the output of this website.
              </p> -->
              <!-- <p>To change the icon of the executable file or create an installer program, download and run <a href="https://github.com/Mixality/packager-extras/releases">Sidekick Packager Extras</a> and select the output of this website.</p> -->
              <p>
                To change the icon of the executable file or create an installer
                program, download and run <a
                  href="https://github.com/Menersar/sidekick-packager-extras/releases"
                  >Sidekick Packager Extras</a
                > and select the output of this website.
              </p>
            </div>
          {:else if $options.target.includes("mac")}
            <div>
              <h2>macOS</h2>
              <p>
                Due to Apple policy, packaging for their platforms is
                troublesome. You either have to:
              </p>
              <ul>
                <li>
                  Instruct users to ignore scary Gatekeeper warnings by opening
                  Finder > Navigating to the application > Right click > Open >
                  Open. This website generates applications that require this
                  workaround.
                </li>
                <li>
                  Or pay Apple $100/year for a developer account to sign and
                  notarize the app (very involved process; reach out in feedback
          ...
          {:else if $options.target.includes("linux")}
            <div>
              ...
              <p>
                ...
                and ARM devices such as Raspberry Pis unfortunately are not
                supported yet.
              </p>
            </div>
          {/if}
          ...
          {#if $options.target.includes("electron")}
            <div>
              <h2>Electron</h2>
              <p>
                The Electron environment works by embedding a copy of Chromium
                (the open source part of Google Chrome) along with your project,
          ...
          {:else if $options.target.includes("nwjs")}
            <div>
              <h2>NW.js</h2>
              <p class="warning">
                NW.js support is deprecated and may be removed in the future.
                Use the Electron environments instead. They're better in every
                way.
              </p>
              <p>
                The NW.js environment works by embedding a copy of Chromium (the
                open source part of Google Chrome) along with your project,
                which means the app will be very large.
              </p>
              <p>
                For further help and steps, see <a
                  href="https://docs.nwjs.io/en/latest/For%20Users/Package%20and%20Distribute/#linux"
                  >NW.js Documentation</a
                >.
              </p>
              {#if $options.target.includes("mac")}
                <p>
                  On macOS, the app will run using Rosetta on Apple Silicon
                  Macs.
                </p>
              {/if}
            </div>
          {:else if $options.target.includes("webview-mac")}
            <div>
              <h2>WKWebView</h2>
              <p>
                WKWebView is the preferred way to package for macOS. It will be
                hundreds of MB smaller than the other macOS-specific
                environments and typically run the fastest.
              </p>
              <p>
                The app will run natively on both Intel and Apple silicon Macs
                running macOS 10.13 or later.
              </p>
              <p>Note that:</p>
              <ul>
                <li>
                  Video sensing and loudness blocks will only work in macOS 12
                  or later.
                </li>
              ...
              <p>
                Use the "Electron macOS Application" (inside Other environments)
                or "Plain HTML" environments instead if you encounter these
                issues.
              </p>
            </div>
          {/if}
...
<Section>
  <DropArea on:drop={(e) => importOptionsFromDataTransfer(e.detail)}>
    <div class="buttons">
      <div class="button">
        <Button
          on:click={exportOptions}
          secondary
          text={$_("options.export")}
        />
      </div>
      <div class="button">
        <Button
          on:click={importOptions}
          secondary
          text={$_("options.import")}
        />
      </div>
